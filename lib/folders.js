...
// Folders: a todo belongs to at most one folder, stored as a path string with
// "/" between levels ("Work/Projects/Alpha"). Nesting is implied by the path, so
// a folder exists when a todo uses it or when it was created explicitly
...
// Accepts an array of segments too (the importer may pass segments).
...
// ({ path, created }); todos contribute their `folder`. Ancestors are filled in so
// the tree is always complete. Sorted by path, case-insensitive, so children follow
// their parent.
function listFolders(todos, explicit) {
  ...
  for (const rec of explicit || []) {
    ...
    if (!p) continue;
    for (const a of ancestorsOf(p)) touch(a, a === p ? (rec.created || rec.createdAt) : null);
  }
